import { AlertDialog, AlertDialogBody, AlertDialogContent, AlertDialogFooter, AlertDialogHeader, AlertDialogOverlay, Box, Button, FormControl, FormErrorMessage, FormHelperText, FormLabel, Image, Input, InputGroup, InputLeftElement, Textarea, VStack } from "@chakra-ui/react";
...
export default function MintNftDialog({ mintNftDialogVisible, setMintNftDialogVisible, progressMint, mintNft }: MintNftDialog) {
    ...
    const onCrop = useCallback((event: Cropper.CropEvent<HTMLImageElement>) => {
        setCropperInstance(event.currentTarget.cropper);
    }, [])
            ...
            <AlertDialogOverlay>
                <AlertDialogContent>
                            ...
                            <form onSubmit={formik.handleSubmit}>
                                ...
                                <VStack gap="4">
                                    <FormControl isInvalid={!!formik.errors.name} isRequired>
                                        ...
                                        <Input colorScheme="brand" name="name" value={formik.values.name} onChange={formik.handleChange} placeholder="My awesome NFT" />
                                    ...
                                    <FormControl isInvalid={!!formik.errors.image} isRequired>
                                        ...
                                        {(formik.values.image?.name !== "" && !isCropped) ?
                                            <>
                                                <Cropper src={formik.values.imageUploadUri} aspectRatio={1} guides style={{ width: "100%", height: "20rem" }} crop={onCrop} accept={nftImageTypesAccepted} alt="Image cropper" />
                                            ...
                                            <>
                                                {formik.values.imageUploadUri !== "" &&
                                                    <Image src={formik.values.imageUploadUri} alt="cropped nft image" width="full" cursor="pointer" onClick={() => { fileUploadInputRef.current?.click() }} />
                                                }
                                                ...
                                                <InputGroup minHeight="3rem" display="flex" alignItems="center">
                                                    ...
                                                    <Input cursor="pointer" onClick={() => { fileUploadInputRef.current?.click() }} placeholder="Upload" value={formik.values.image?.name || "No file selected"} readOnly variant={isCropped ? "unstyled" : "outline"} />
                    ...
                    <AlertDialogFooter gap="4">
                        ...
                        <Button type="submit" colorScheme="brand" onClick={() => { formik.handleSubmit() }} isLoading={progressMint} loadingText="Minting" isDisabled={!formik.isValid}>
                            Mint
                        </Button>
                    </AlertDialogFooter>
